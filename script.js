...
const locations = {
    bridge: {
        name: 'Wah Fu Circular Bridge (華�?�?��??形天�?)',
        lat: 22.25216,
        lon: 114.13635,
        icon: 'info-sign',
        color: 'blue',
        description: '華�?�?��?�?�?��??�??形天�?�?�?��?�不�?�?�??�??�?�要�??�?�??',
        image: 'assets/�??形天�?.png',
        badge: 'Architecture',
        title: '�??形天�?',
        subtitle: '華�?�?��?�?�?��??�??形天�?',
    ...
    housing: {
        name: 'Twin Tower Housing - Wah Som House (�?�?�?��?)',
        lat: 22.25135,
        lon: 114.13475,
        icon: 'home',
        color: 'orange',
    ...
    park: {
        name: 'Waterfall Bay Park (�??�?灣�?��??)',
        lat: 22.250556913543175,
        lon: 114.13481016703993,
        icon: 'tree-conifer',
        color: 'green',
        description: '設�??不�?�?�度�??路�?�?壯�?山海�?��?��?�??示�?�?��??山�?�?建議預�?? 1 �?� 2 小�??�??',
...
const timelineData = [
    {
        title: '第�?�??段�?遷置�??�?�??',
        description: '�?�?�?��?遷�?�?��?��?�?��?華�?�??�?�??�??�?�平�?��?�?�??工�?�?�?��?��?��?�?好�?�??�??'
    },
    {
        title: '第�?�??段�?�?��?建設',
        description: '�?��?�?路�?�水�?��?��??�?�?�?��?設�?�建設�?�?��?�社�?奠�?�?��?�??'
    },
    {
        title: '第�?�??段�?建�?�??落�?�',
        description: '�??建�?�式住�?大�?�?�?��?設�?��?預�?�?�?��?可容納�?��?�?�?�??'
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ====== DEEPFAKE SITE �?? TABS (Bright Side) ======
    ...
    if (tabButtons.length > 0) {
        tabButtons.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                btn.classList.add('active');
                const target = document.querySelector(`.tab-panel[data-tab="${key}"]`);
                if (target) target.classList.add('active');
            });
        });
    }

    // ====== DEEPFAKE SITE �?? ACCORDIONS (Myths) ======
    ...
    // ====== DEEPFAKE SITE �?? CHECKLIST (Armor) ======
    const armorItems = document.querySelectorAll('.armor-item');
    const armorFill = document.querySelector('.armor-progress-fill');
    const armorNum = document.querySelector('.armor-progress-num');
    function updateArmorProgress() {
        if (armorItems.length === 0) return;
        const checked = document.querySelectorAll('.armor-item.checked').length;
        const pct = Math.round((checked / armorItems.length) * 100);
        if (armorFill) armorFill.style.width = pct + '%';
        if (armorNum) armorNum.textContent = pct + '%';
    }
    ...
    if (armorItems.length > 0) updateArmorProgress();

    // ====== DEEPFAKE SITE ? PREVENTION STEPPER (4-step navigator) ======
    const prevStepper = document.getElementById('preventionStepper');
    if (prevStepper) {
        const steps = prevStepper.querySelectorAll('.prev-step');
        const dots = prevStepper.querySelectorAll('.prev-dot');
        const prevBtn = prevStepper.querySelector('.prev-step-prev');
        const nextBtn = prevStepper.querySelector('.prev-step-next');
        const currentLabel = prevStepper.querySelector('.prev-step-current');
        const totalLabel = prevStepper.querySelector('.prev-step-total');
        ...
        function showStep(n) {
            currentStep = Math.max(1, Math.min(total, n));
            steps.forEach(step => {
                const isActive = parseInt(step.getAttribute('data-step'), 10) === currentStep;
                step.classList.toggle('active', isActive);
            });
            dots.forEach(dot => {
                const isActive = parseInt(dot.getAttribute('data-step'), 10) === currentStep;
        ...
        // Keyboard navigation: ? / ? when stepper is in view
        document.addEventListener('keydown', (e) => {
            const rect = prevStepper.getBoundingClientRect();
    ...
    // ====== DEEPFAKE SITE �?? REFERENCES FILTER ======
    const refFilters = document.querySelectorAll('.refs-filter button');
    const refItems = document.querySelectorAll('.ref-item');
    refFilters.forEach(btn => {
        btn.addEventListener('click', () => {
            const cat = btn.getAttribute('data-cat');
            refFilters.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            refItems.forEach(item => {
                if (cat === 'all' || item.getAttribute('data-cat') === cat) {
                    item.classList.remove('ref-item-hidden');
                } else {
                    item.classList.add('ref-item-hidden');
                }
            });
        });
    });

    // ====== DEEPFAKE SITE �?? GENESIS CHART (Chart.js) ======
    const chartCanvas = document.getElementById('proliferationChart');
    if (chartCanvas && typeof Chart !== 'undefined') {
        const ctx = chartCanvas.getContext('2d');
        const gradient = ctx.createLinearGradient(0, 0, 0, 400);
        gradient.addColorStop(0, 'rgba(43,127,255,0.45)');
        gradient.addColorStop(1, 'rgba(43,127,255,0)');

        new Chart(ctx, {
            type: 'line',
            data: {
                labels: ['2017', '2019', '2021', '2023', '2024', '2025', '2027 (est.)', '2031 (est.)'],
                datasets: [
                    {
                        label: 'Generative AI Market Value (USD Billions)',
                        data: [1, 5, 12, 40, 95, 180, 260, 442],
                        borderColor: '#2b7fff',
                    ...
                    {
                        label: 'US Fraud Losses (USD Billions)',
                        data: [0.5, 1.2, 3, 12.3, 18, 25, 40, null],
                        borderColor: '#dc2626',
